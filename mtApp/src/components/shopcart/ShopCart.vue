...
<div class="shopcart" :class="{'highligh':totalCount > 0}">
    ...
    <div class="content-left">
			<div class="logo-wrapper" :class="{'highligh':totalCount > 0}">
				<span class="icon-shopping_cart logo" :class="{'highligh':totalCount > 0}" @click="toggleList">
				...
				<ul>
					<li 
						...
						<div class="cartcontrol-wrapper">
							<app-cart-control :item="food"></app-cart-control>
						</div>
					</li>
				</ul>
			</div>
      ...
      selectFoods:{
		...
		computed:{
			...
			totalPrice(){
				let totalSell = 0
				this.selectFoods.forEach((food) => {
					totalSell += food.count * food.min_price
				});
				return totalSell
			},
			payStr(){
...
.shopcart-wrapper .content-left .logo-wrapper i{
	width: 15px;
	height: 15px;
	line-height: 15px;
	border-radius: 50%;
